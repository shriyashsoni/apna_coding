  ...
  const handleCreate = async () => {
    ...
    try {
      const { error } = await supabase.from('event_groups').insert({
        ...
        start_date: formData.startDate ? new Date(formData.startDate).getTime() : Date.now(),
        end_date: formData.endDate ? new Date(formData.endDate).getTime() : Date.now() + 86400000,
