...
export function AIEmailAgent() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (resData && resData.success && resData.templates) {
        const templates = resData.templates;
        setGeneratedEmail({
        ...
        if (resData.provider === "fallback") {
          toast.success("✨ Custom templates prepared successfully!");
        } else {
          toast.success(`✨ Three personalized email variations successfully generated via ${resData.provider === "grok" ? "Grok-2" : "Gemini"}!`);
        }
      } else {
        throw new Error("Invalid response schema received from API.");
      }
    } catch (err: any) {
      console.error("AI Generation Error:", err);
      toast.error(`AI Generation Failed: ${err.message || "Failed to reach AI server."}`);
    } finally {
